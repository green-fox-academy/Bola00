...
for ( int i = 0; i <students.size(); i++ ){
    if (students[i].second.at("candies") > 4){ //check the students maps values, if the the pair second part's candy part is more than 4
        children.push_back(students[i].first);// give the children name (which is the map's first part(key))
    }
} return children;
}
...
int sumOfAgeWithLessThanFiveCandies(const std::vector<std::pair<std::string, std::map<std::string, int>>>& students) {
    ...
    for (int i = 0; i < students.size(); i++) {
        if (students[i].second.at("candies") < 5) {
            age_sum += students[i].second.at("age");
        }
    }return age_sum;
}
